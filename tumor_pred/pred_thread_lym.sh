...
DATA_FILE=prediction_baseline.txt
...
while [ ${REPEAT_COUNT} -lt ${MAX_REPEAT} ]; do
    ...
    for files in ${FOLDER}/*; do
        ...
        LINE_N=$((LINE_N+1))
        NEWPATH="../prediction_values/$(basename "${files}" .svs)_prediction_baseline.txt"
        ...
        if [ -f "${files}/${DONE_FILE}" ]; then
            if [ ! -f "${NEWPATH}" ]; then
                ...
                python -u ${EXEC_FILE} ${files} ../model/ ${DATA_FILE} ../model/model_base.t7
            fi
        fi
    done
